...
print(f"Using Gemini-compatible endpoint | model: {ACTIVE_MODEL}")
...
def _get_llm(temperature: float = 0) -> ChatOpenAI:
    ...
    return ChatOpenAI(
        ...
        openai_api_base=ACTIVE_BASE_URL,
    )
...
async def query_data(request: QueryRequest, current_user: dict = Depends(get_current_user)):
    ...
    def build_response(text: str, html_content: str = None) -> dict:
        is_image, image_file = get_single_image_base64(IMAGES_FOLDER)
        # empty_folder(IMAGES_FOLDER)
        if is_image and not text.strip():
            text = f"Here is the chart for your query: {query}"
        resp = {"response": text, "is_images": is_image, "image_file": image_file}
    ...
    # ─────────────────────────────────────────────
    # NON-CHART PATH — agent
    # ─────────────────────────────────────────────
    context = "\n".join([
        f"Previous User Query: {e['query']}\nPrevious Bot Response: {e['response']}"
        for e in CHAT_HISTORY[-MAX_HISTORY_LENGTH:]
    ])

    if is_table_request:
        query_format = (
            "Explicit table request: start with a 2-4 sentence summary, "
            "then provide a markdown table."
        )
    elif is_list_request:
        query_format = "Format as a concise numbered list with a brief intro, NOT a table."
    elif is_time_comparison:
        query_format = (
            "Time comparison query: start with 2-4 sentence summary, "
            "then a markdown table with a % change column."
        )
    else:
        query_format = (
            "For complex data: 2-4 sentence description followed by "
...
Steps:
1. Understand which aspects of the data are relevant.
2. Identify useful fields from the schema.
...
Rules:
- For tables: start with a 2-4 sentence text summary BEFORE the table.
- For YoY/MoM: always include % changes with + or - prefixes.
- If the user explicitly asks for a table, return a table even for top/list-style queries.
- For lists: use numbered list format, not a table, unless a table was requested.
- For tabular data: use markdown table with | separators, header row, and separator row.

CHART INSTRUCTIONS (if generating a chart):
- Save to '{IMAGES_FOLDER}' directory as a .png file.
- Write a 1-2 sentence text summary after saving.
- Do NOT display or embed the chart in text output.
"""

    try:
        # _get_llm() enforces rate limits
        llm   = _get_llm(temperature=0)
        ...
        response_text = res.get("output") if isinstance(res, dict) else (str(res) if res else "Could not find an answer.")
